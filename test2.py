...
def combat_boss_final (hp_ennemi, nom_poke_ennemi, degat1, degat2, degat3, pokemon_stats):
	
	if pokemon_stats["starter"] in (4,5,6):
		pokemon_stats["hp_starter"] += 40
		print(f"Que le combat commence !")
	elif pokemon_stats["starter"] in (7,8,9):
		pokemon_stats["hp_starter"] += 70
		print(f"Que le combat commence !")
	elif pokemon_stats["starter"] in (10, 11, 12, 13):
		pokemon_stats["hp_starter"] += 100
		print(f"Que le combat commence !")
	else:
	    print(f"Que le combat commence !")
	while hp_ennemi > 0 and pokemon_stats["hp_starter"] > 0 and not pokemon_stats["fuir"] and not pokemon_stats["attraper"]:
		...
		if not pokemon_stats["attraper"]:
                        if hp_ennemi > 0 :
                                ...
                                if attaque_ennemi == 1:
                                        pokemon_stats["hp_starter"] -= degat1
                                        print(f"\n{nom_poke_ennemi} vous a infligé {degat1} dégats. Vous avez {pokemon_stats["hp_starter"]} points de vie.")
                                elif attaque_ennemi == 2:
                                        pokemon_stats["hp_starter"] -= degat2
                                        print(f"\n{nom_poke_ennemi} vous a infligé {degat2} dégats. Vous avez {pokemon_stats["hp_starter"]} points de vie.")
                                elif attaque_ennemi == 3:
                                        pokemon_stats["hp_starter"] -= degat3
                                        print(f"\n{nom_poke_ennemi} vous a infligé {degat3} dégats. Vous avez {pokemon_stats["hp_starter"]} points de vie.")
                                else:
                        ...
                        if pokemon_stats["attraper"] == True:
                                print(f"\nTu as attrapé le Pokémon ! Bravo. Ça veut dire que je peux gagner tous mes combats en attrapant les pokémons adverses ? Imagine pokemon_stats["attraper"] un Pokémon destabiliserait l'île et tuerait tous les habitants. Nan je rigole mais imagine. ")
                                #faire texte manquant !!!!! si pokemon pokemon_stats["attraper"] = mort aussi 
                        if pokemon_stats["hp_starter"] <= 0:
                                ...
                                input("↓")
                                print(f" {prenom} : Vite ! J-je.. je dois pokemon_stats["fuir"] !!")
                                input("↓")
                        ...
                        if hp_ennemi <= 0:
                                pokemon_stats["monnaie"]_gagne = random.randint(40,50)
                                pokemon_stats["monnaie"] += pokemon_stats["monnaie"]_gagne
                                print(f"Vous gagnez {pokemon_stats["monnaie"]_gagne} pièces. Vous avez maintenant {pokemon_stats["monnaie"]} pièces.")
                                input("↓")
                                ...
                                victoire(pokemon_stats)
#Fonction combat
def boucle_combat (hp_ennemi, nom_poke_ennemi, degat1, degat2, degat3, pokemon_stats):
        ...
        while hp_ennemi > 0 and pokemon_stats["hp_starter"] > 0 and not pokemon_stats["fuir"] and not pokemon_stats["attraper"]:
            ...
            if pokemon_stats["hp_starter"] <= 0:
                    ...
                    pokemon_stats["monnaie"]_gagne = random.randint(20,30)
                    pokemon_stats["monnaie"] += pokemon_stats["monnaie"]_gagne
                    print(f"\nVous gagnez quand même {pokemon_stats["monnaie"]_gagne} pièces. Vous avez maintenant {pokemon_stats["monnaie"]} pièces.")
            ...
            if hp_ennemi <= 0:
                    pokemon_stats["XP"]_gagne=random.randint(100,200)
                    pokemon_stats["XP"] += pokemon_stats["XP"]_gagne
                    pokemon_stats["monnaie"]_gagne = random.randint(50,60)
                    pokemon_stats["monnaie"] += pokemon_stats["monnaie"]_gagne
                    ...
                    if pokemon_stats["XP"] >= 500:
                        ...
                        print(f"\nFélicitation, vous avez gagné(e) votre combat face à {nom_poke_ennemi}. Vous gagnez {pokemon_stats["monnaie"]_gagne} pièces. Vous avez maintenant {pokemon_stats["monnaie"]} pièces. Vous avez gagné(e) {pokemon_stats["XP"]} pokemon_stats["XP"]s !")
                        return (pokemon_stats["XP"], starter, pokemon_stats["hp_starter"], pokemon_stats["state_combat"], pokemon_stats["nbr_attaque1"], pokemon_stats["nbr_attaque3"], pokemon_stats["nbr_attaque3"], pokemon_stats["nbr_attaque1"], pokemon_stats["nbr_attaque3"], pokemon_stats["nbr_attaque3"], pokemon_stats["nbr_attaque1"], pokemon_stats["nbr_attaque3"], pokemon_stats["nbr_attaque3"], pokemon_stats["nbr_attaque1"], pokemon_stats["nbr_attaque3"], pokemon_stats["nbr_attaque3"])
	...
	if choix == 1: 
		...
		if choixsoin == 1: 
            ...
            if choix_pokeball == 1:
                ...
                if pokemon_stats["nbr_hyperball"] > 0: 
                    pokemon_stats["nbr_hyperball"] -= 1
            ...
            print(f"Vous ne voullez pas combattre car vous êtes une énorme tapette et décidez de pokemon_stats["fuir"] le combat !")
...
def menu_combat_arcko (pokemon_stats)
	...
	while not  choix in (1,2,3,4):
		...
		choix=int(input("choisissez le bon numéro "))
	if choix == 1: 
		print(f"Quelle attaque voulez vous utiliser ? \n 1)Désherbaffe (-15 hp) {pokemon_stats["nbr_attaque1"]}/15 \n 2)Fouets-Lianness (-10 hp) {pokemon_stats["nbr_attaque3"]}/20 \n 3)Lamme-Feuille (-20 hp) {pokemon_stats["nbr_attaque3"]}/10 ")
		quelle_attaque=int(input("choisissez le bon numéro "))
		while not quelle_attaque in (1,2,3):
			print(f"Veuillez saisir un nombre correct")
			print(f"Quelle attaque voulez vous utiliser ? \n 1)Désherbaffe (-15 hp) {pokemon_stats["nbr_attaque1"]}/15 \n 2)Fouets-Liannes (-10 hp) {pokemon_stats["nbr_attaque3"]}/20 \n 3)Lamme-Feuille (-20 hp) {pokemon_stats["nbr_attaque3"]}/10 ")
			quelle_attaque=int(input("choisissez le bon numéro "))
		if quelle_attaque == 1:
			if pokemon_stats["nbr_attaque1"] > 0:
				hp_ennemi -= 15 
        ...
        if achat_boutique == 1:
                ...
                if achat_poke == 1:
                        ...
                        if pokemon_stats["monnaie"] >= 10 :
                                ...
                                pokemon_stats["monnaie"] -= 10
                                pokemon_stats["nbr_superball"] += 1
                        else :
                                print(f"T'as plus de thunes sale sdf, sors de ma boutique !")
                else :
                        if pokemon_stats["monnaie"] >= 15 :
                                print(f"Vous venez d'acheter une hyperball !")
                                pokemon_stats["monnaie"] -= 15
                                pokemon_stats["nbr_hyperball"] += 1
                        else :
                                print(f"T'as plus de thunes sale sdf, sors de ma boutique")
...
def ville_donjon (pokemon_stats):
        ...
        print(f"Que le combat commence")
        #combat ténéfix
        combat1 = boucle_combat(65, "Ténéfix", 15, 10, 20, pokemon_stats)
        pokemon_stats["XP"] = combat1 [0]
        pokemon_stats["starter"] = combat1 [1]
        pokemon_stats["hp_starter"] = combat1 [2]
        pokemon_stats["hp_starter"] = 60
        pokemon_stats["nbr_attaque1"] = combat1 [4]
        pokemon_stats["nbr_attaque3"] = combat1 [5]
        pokemon_stats["nbr_attaque3"] = combat1 [6]
        pokemon_stats["nbr_attaque1"] = combat1 [7]
        pokemon_stats["nbr_attaque3"] = combat1 [8]
        pokemon_stats["nbr_attaque3"] = combat1 [9]
        pokemon_stats["nbr_attaque1"] = combat1 [10]
        pokemon_stats["nbr_attaque3"] = combat1 [11]
        pokemon_stats["nbr_attaque3"] = combat1 [12]
        pokemon_stats["nbr_attaque1"] = combat1 [13]
        pokemon_stats["nbr_attaque3"] =	combat1 [14]
        pokemon_stats["nbr_attaque3"] = combat1 [15]
        pokemon_stats["nbr_attaque1"] = 15
        pokemon_stats["nbr_attaque3"] = 20
        pokemon_stats["nbr_attaque3"] = 10
        pokemon_stats["nbr_attaque1"] = 15
        ...
        if choix_porte == 1 :
                ...
                print(f"Que le combat commence !")
                #combat absol
                combat2 = boucle_combat(60, "Absol", 15, 10, 20, pokemon_stats)
                pokemon_stats["XP"] = combat2 [0]
                pokemon_stats["starter"] = combat2 [1]
                pokemon_stats["hp_starter"] = combat2 [2]
                pokemon_stats["hp_starter"] = 60
                pokemon_stats["nbr_attaque1"] = combat2 [4]
                pokemon_stats["nbr_attaque3"] = combat2 [5]
                pokemon_stats["nbr_attaque3"] = combat2 [6]
                pokemon_stats["nbr_attaque1"] = combat2 [7]
                pokemon_stats["nbr_attaque3"] = combat2 [8]
                pokemon_stats["nbr_attaque3"] = combat2 [9]
                pokemon_stats["nbr_attaque1"] = combat2 [10]
                pokemon_stats["nbr_attaque3"] = combat2 [11]
                pokemon_stats["nbr_attaque3"] = combat2 [12]
                pokemon_stats["nbr_attaque1"] = combat2 [13]
                pokemon_stats["nbr_attaque3"] =	combat2 [14]
                pokemon_stats["nbr_attaque3"] = combat2 [15]
                pokemon_stats["nbr_attaque1"] = 15
                pokemon_stats["nbr_attaque3"] = 20
                pokemon_stats["nbr_attaque3"] = 10
                pokemon_stats["nbr_attaque1"] = 15
                ...
                print(f"Que le combat commence !")
                #combat spectrum
                combat3 = boucle_combat(75, "Spectrum", 15, 10, 20, pokemon_stats)
                pokemon_stats["XP"] = combat3 [0]
                pokemon_stats["starter"]= combat3 [1]
                pokemon_stats["hp_starter"] = combat3 [2]
                pokemon_stats["hp_starter"] = 60
                pokemon_stats["nbr_attaque1"] = combat3 [4]
                pokemon_stats["nbr_attaque3"] = combat3 [5]
                pokemon_stats["nbr_attaque3"] = combat3 [6]
                pokemon_stats["nbr_attaque1"] = combat3 [7]
                pokemon_stats["nbr_attaque3"] = combat3 [8]
                pokemon_stats["nbr_attaque3"] = combat3 [9]
                pokemon_stats["nbr_attaque1"] = combat3 [10]
                pokemon_stats["nbr_attaque3"] = combat3 [11]
                pokemon_stats["nbr_attaque3"] = combat3 [12]
                pokemon_stats["nbr_attaque1"] = combat3 [13]
                pokemon_stats["nbr_attaque3"] = combat3 [14]
                pokemon_stats["nbr_attaque3"] = combat3 [15]
                pokemon_stats["nbr_attaque1"] = 15
                pokemon_stats["nbr_attaque3"] = 20
                pokemon_stats["nbr_attaque3"] = 10
                pokemon_stats["nbr_attaque1"] = 15
                ...
                if choix_trappe == 1:
                        ...
                        while True:
                                ...
                                if position == "S":
                                        ...
                                        boss_donjon(pokemon_stats)
                                        break
                                if all(direction is None for direction in chemin.values()):
...
def boss_donjon (pokemon_stats):
        #combat trioxhydre
        combat7 = boucle_combat(100, "Trioxhydre", 15, 10, 20, pokemon_stats)
        pokemon_stats["XP"] = combat7 [0]
        pokemon_stats["starter"]= combat7 [1]
        pokemon_stats["hp_starter"] = combat7 [2]
        pokemon_stats["hp_starter"] = 60
        pokemon_stats["nbr_attaque1"] = combat7 [4]
        pokemon_stats["nbr_attaque3"] = combat7 [5]
        pokemon_stats["nbr_attaque3"] = combat7 [6]
        pokemon_stats["nbr_attaque1"] = combat7 [7]
        pokemon_stats["nbr_attaque3"] = combat7 [8]
        pokemon_stats["nbr_attaque3"] = combat7 [9]
        pokemon_stats["nbr_attaque1"] = combat7 [10]
        pokemon_stats["nbr_attaque3"] = combat7 [11]
        pokemon_stats["nbr_attaque3"] = combat7 [12]
        pokemon_stats["nbr_attaque1"] = combat7 [13]
        pokemon_stats["nbr_attaque3"] =	combat7 [14]
        pokemon_stats["nbr_attaque3"] = combat7 [15]
        pokemon_stats["nbr_attaque1"] = 15
        pokemon_stats["nbr_attaque3"] = 20
        pokemon_stats["nbr_attaque3"] = 10
        pokemon_stats["nbr_attaque1"] = 15
...
def arènes_pokémons (pokemon_stats):
        ...
        if response_8 == 1:
                ...
                pokemon_stats["hp_starter"] = 60
                pokemon_stats["state_combat"] = arene1 [3]
                pokemon_stats["nbr_attaque1"] = arene1 [4]
                pokemon_stats["nbr_attaque3"] = arene1 [5]
                pokemon_stats["nbr_attaque3"] = arene1 [6]
                pokemon_stats["nbr_attaque1"] = arene1 [7]
                pokemon_stats["nbr_attaque3"] = arene1 [8]
                pokemon_stats["nbr_attaque3"] = arene1 [9]
                pokemon_stats["nbr_attaque1"] = arene1 [10]
                pokemon_stats["nbr_attaque3"] = arene1 [11]
                pokemon_stats["nbr_attaque3"] = arene1 [12]
                pokemon_stats["nbr_attaque1"] = arene1 [13]
                pokemon_stats["nbr_attaque3"] =	arene1 [14]
                pokemon_stats["nbr_attaque3"] = arene1 [15]
                if pokemon_stats["state_combat"] == True:
                        print("Bravo, vous passez au dresseur suivant !")
                        print(f"votre deucième combat commence !")
                        pokemon_stats["state_combat"] = False
                        arene2 = boucle_combat(70, "Malosse", 15, 10, 20, pokemon_stats)
                        pokemon_stats["XP"] = arene2 [0]
                        pokemon_stats["starter"]= arene2 [1]
                        pokemon_stats["hp_starter"] = arene2 [2]
                        pokemon_stats["hp_starter"] = 60
                        pokemon_stats["state_combat"] = arene2 [3]
                        pokemon_stats["nbr_attaque1"] = arene2 [4]
                        pokemon_stats["nbr_attaque3"] = arene2 [5]
                        pokemon_stats["nbr_attaque3"] = arene2 [6]
                        pokemon_stats["nbr_attaque1"] = arene2 [7]
                        pokemon_stats["nbr_attaque3"] = arene2 [8]
                        pokemon_stats["nbr_attaque3"] = arene2 [9]
                        pokemon_stats["nbr_attaque1"] = arene2 [10]
                        pokemon_stats["nbr_attaque3"] = arene2[11]
                        pokemon_stats["nbr_attaque3"] = arene2 [12]
                        pokemon_stats["nbr_attaque1"] = arene2 [13]
                        pokemon_stats["nbr_attaque3"] =	arene2 [14]
                        pokemon_stats["nbr_attaque3"] = arene2 [15]
                        if pokemon_stats["state_combat"] == True:
                                print("Bravo, vous passez au dresseur suivant !")
                                print(f"votre premier combat commence !")
                                pokemon_stats["state_combat"] = False
                                arene3 = boucle_combat(80, "Mackogneur", 25, 20, 30, pokemon_stats)
                                pokemon_stats["XP"] = arene3 [0]
                                pokemon_stats["starter"]= arene3 [1]
                                pokemon_stats["hp_starter"] = arene3 [2]
                                pokemon_stats["hp_starter"] = 60
                                pokemon_stats["state_combat"] = arene3 [3]
                                pokemon_stats["nbr_attaque1"] = arene3 [4]
                                pokemon_stats["nbr_attaque3"] = arene3 [5]
                                pokemon_stats["nbr_attaque3"] = arene3 [6]
                                pokemon_stats["nbr_attaque1"] = arene3 [7]
                                pokemon_stats["nbr_attaque3"] = arene3 [8]
                                pokemon_stats["nbr_attaque3"] = arene3 [9]
                                pokemon_stats["nbr_attaque1"] = arene3 [10]
                                pokemon_stats["nbr_attaque3"] = arene3 [11]
                                pokemon_stats["nbr_attaque3"] = arene3 [12]
                                pokemon_stats["nbr_attaque1"] = arene3 [13]
                                pokemon_stats["nbr_attaque3"] =	arene3 [14]
                                pokemon_stats["nbr_attaque3"] = arene3 [15]
                                if pokemon_stats["state_combat"] == True:
                                        print("Bravo, vous passez au dresseur suivant !")
                                        print(f"votre premier combat commence !")
                                        pokemon_stats["state_combat"] = False
                                        arene4 = boucle_combat(90, "Dracofeu", 25, 20, 30, pokemon_stats)
                                        pokemon_stats["XP"] = arene4 [0]
                                        pokemon_stats["starter"]= arene4 [1]
                                        pokemon_stats["hp_starter"] = arene4 [2]
                                        pokemon_stats["hp_starter"] = 60
                                        pokemon_stats["state_combat"] = arene4 [3]
                                        pokemon_stats["nbr_attaque1"] = arene4 [4]
                                        pokemon_stats["nbr_attaque3"] = arene4 [5]
                                        pokemon_stats["nbr_attaque3"] = arene4 [6]
                                        pokemon_stats["nbr_attaque1"] = arene4 [7]
                                        pokemon_stats["nbr_attaque3"] = arene4 [8]
                                        pokemon_stats["nbr_attaque3"] = arene4 [9]
                                        pokemon_stats["nbr_attaque1"] = arene4 [10]
                                        pokemon_stats["nbr_attaque3"] = arene4 [11]
                                        pokemon_stats["nbr_attaque3"] = arene4 [12]
                                        pokemon_stats["nbr_attaque1"] = arene4 [13]
                                        pokemon_stats["nbr_attaque3"] =	arene4 [14]
                                        pokemon_stats["nbr_attaque3"] = arene4 [15]
                                        pokemon_stats["nbr_attaque1"] = 15
                                        pokemon_stats["nbr_attaque3"] = 20
                                        pokemon_stats["nbr_attaque3"] = 10
                                        pokemon_stats["nbr_attaque1"] = 15
...
def forêt_pangorn (pokemon_stats):
        ...
        if response_9 == 1 :
                ...
                if response_10 == 1 :
                        ...
                        if Croco_griffe == 1:
                                ...
                                medecin (pokemon_stats)
                                input("↓")
                                print(f"Le Crocodil se dresse devant vous, préparez vous au combat !")
                                combat10 = boucle_combat(65, "Crocodil", 15, 10, 20, pokemon_stats)
                                pokemon_stats["XP"] = combat10 [0]
                                pokemon_stats["starter"]= combat10 [1]
                                pokemon_stats["hp_starter"] = combat10 [2]
                                pokemon_stats["hp_starter"] = 60
                                pokemon_stats["nbr_attaque1"] = combat10 [4]
                                pokemon_stats["nbr_attaque3"] = combat10 [5]
                                pokemon_stats["nbr_attaque3"] = combat10 [6]
                                pokemon_stats["nbr_attaque1"] = combat10 [7]
                                pokemon_stats["nbr_attaque3"] = combat10 [8]
                                pokemon_stats["nbr_attaque3"] = combat10 [9]
                                pokemon_stats["nbr_attaque1"] = combat10 [10]
                                pokemon_stats["nbr_attaque3"] = combat10 [11]
                                pokemon_stats["nbr_attaque3"] = combat10 [12]
                                pokemon_stats["nbr_attaque1"] = combat10 [13]
                                pokemon_stats["nbr_attaque3"] = combat10 [14]
                                pokemon_stats["nbr_attaque3"] = combat10 [15]
                                pokemon_stats["nbr_attaque1"] = 15
                                pokemon_stats["nbr_attaque3"] = 20
                                pokemon_stats["nbr_attaque3"] = 10
                                pokemon_stats["nbr_attaque1"] = 15
                ...
                #combat
                combat5 = boucle_combat(65, "Ouisticram", 15, 10, 20, pokemon_stats)
                pokemon_stats["XP"] = combat5 [0]
                pokemon_stats["starter"]= combat5 [1]
                pokemon_stats["hp_starter"] = combat5 [2]
                pokemon_stats["hp_starter"] = 60
                pokemon_stats["nbr_attaque1"] = combat5 [4]
                pokemon_stats["nbr_attaque3"] = combat5 [5]
                pokemon_stats["nbr_attaque3"] = combat5 [6]
                pokemon_stats["nbr_attaque1"] = combat5 [7]
                pokemon_stats["nbr_attaque3"] = combat5 [8]
                pokemon_stats["nbr_attaque3"] = combat5 [9]
                pokemon_stats["nbr_attaque1"] = combat5 [10]
                pokemon_stats["nbr_attaque3"] = combat5 [11]
                pokemon_stats["nbr_attaque3"] = combat5 [12]
                pokemon_stats["nbr_attaque1"] = combat5 [13]
                pokemon_stats["nbr_attaque3"] = combat5 [14]
                pokemon_stats["nbr_attaque3"] = combat5 [15]
                pokemon_stats["nbr_attaque1"] = 15
                pokemon_stats["nbr_attaque3"] = 20
                pokemon_stats["nbr_attaque3"] = 10
                pokemon_stats["nbr_attaque1"] = 15
                ...
                if response_13 == 1:
                        ...
                        medecin (pokemon_stats)
                        onix_secret(pokemon_stats)
def onix_secret (pokemon_stats):
        print(f"Il va falloir combattre pour savoir ce qui intrigue ce onix !")
        #combat onix
        combat6 = boucle_combat(70, "Onix", 15, 10, 20, pokemon_stats)
        pokemon_stats["XP"] = combat6 [0]
        pokemon_stats["starter"]= combat6 [1]
        pokemon_stats["hp_starter"] = combat6 [2]
        pokemon_stats["hp_starter"] = 60
        pokemon_stats["nbr_attaque1"] = combat6 [4]
        pokemon_stats["nbr_attaque3"] = combat6 [5]
        pokemon_stats["nbr_attaque3"] = combat6 [6]
        pokemon_stats["nbr_attaque1"] = combat6 [7]
        pokemon_stats["nbr_attaque3"] = combat6 [8]
        pokemon_stats["nbr_attaque3"] = combat6 [9]
        pokemon_stats["nbr_attaque1"] = combat6 [10]
        pokemon_stats["nbr_attaque3"] = combat6 [11]
        pokemon_stats["nbr_attaque3"] = combat6 [12]
        pokemon_stats["nbr_attaque1"] = combat6 [13]
        pokemon_stats["nbr_attaque3"] = combat6 [14]
        pokemon_stats["nbr_attaque3"] = combat6 [15]
        pokemon_stats["nbr_attaque1"] = 15
        pokemon_stats["nbr_attaque3"] = 20
        pokemon_stats["nbr_attaque3"] = 10
        pokemon_stats["nbr_attaque1"] = 15
...
while chemins:
    ...
    if choix in chemins:
        ...
        print(f"\nTu as choisi : {nom}\n")
        fonction(pokemon_stats)  #  Lance la fonction liée
